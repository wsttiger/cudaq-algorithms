...
Two-level validation, mirroring the sparse-oracle tests: the circuit's
index-register marginal (amplitude magnitudes squared, summed over every
garbage configuration) is pinned at simulator precision against an
*independent* brute-force enumeration of the classical keep/alias branches
— for bin k and reference sigma the branch lands on k if sigma < keep[k]
else on alias[k], each branch carrying probability 1 / (K 2^mu) — and that
exact table distribution is pinned against the ideal w / lambda within the
derived discretization bound (integer weights W_k = round(p_k K 2^mu)
plus at most one unit of residual redistribution: |W_k - ideal| <= 3/2, so
...
from cudaq_algorithms.sparse import AliasSamplingPrepare
